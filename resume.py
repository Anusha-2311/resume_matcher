...
# Predict suitable job roles (ranked by score)
def predict_roles(text):
    ...
    return [role for role, score in sorted_roles if score > 0]
...
uploaded_resume = st.file_uploader("📤 Step 1: Upload Resume (PDF or DOCX)", type=["pdf", "docx"])

if uploaded_resume:
    raw_text = extract_text(uploaded_resume)
    sections = extract_sections(raw_text)
    predicted_roles = predict_roles(raw_text)

    if not predicted_roles:
        st.error("❌ No suitable job roles found from this resume.")
    else:
        st.markdown("### ✅ Suitable Job Roles Based on Resume:")
        for role in predicted_roles:
            st.markdown(f"- **{role}**")

        # JD Input
        jd_text = st.text_area("📝 Step 2: Paste Job Description (JD) to compare", height=300)

        if jd_text:
            selected_role = predicted_roles[0]
            jd_lower = jd_text.lower()

            if selected_role.lower() in jd_lower:
                st.success("✅ JD and Resume appear to align with the predicted role!")

                score = compare_skills(jd_text, sections["Skills"])
                feedback = get_feedback(score)

                st.markdown(f"### 🏷️ Predicted Best Match Role: `{selected_role}`")
                st.markdown(f"### 📊 Match Score: **{score}%**")
                st.markdown(f"### 💡 Feedback: *{feedback}*")
            else:
                st.warning("⚠️ JD may not align well with the predicted roles from the resume. Please review or try a different JD.")
else:
    st.info("Upload a resume to start the prediction process.")
